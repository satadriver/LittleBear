...
int FindExeFile(char * strpath,char * lpfn)
{
	...
	{   
		if ( (stWfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && 
			(lstrcmpiA(stWfd.cFileName,szLastDir) == 0 || lstrcmpiA(stWfd.cFileName,".") == 0) )
		{
			continue; 
		}
...
int MainProc(){
	...
	wsprintfA(szlog,"current module file name:%s\r\n",path);
	WriteLog(szlog);
	...
	if (ret)
	{
		...
		wsprintfA(szlog,"find and run exe file:%s in path:%s\r\n", szexefn,szcurdir);
		WriteLog(szlog);
		return TRUE;
	}
...
int __stdcall DllMain( HANDLE hDllHandle, DWORD dwReason, LPVOID lpreserved ){
	...
	if (dwReason == DLL_PROCESS_ATTACH )
	{
		if (dwflag == FALSE)
		{
			...
			MainProc();
			WriteLog("firload.dll run first time\r\n");
		}
	}
