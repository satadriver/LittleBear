...
int GetDiskSpaceInfo(char * szBuf)
{
	...
	for(int i = 0; i < iLen / 4 ; ++i)
	{
		...
		if (iRes == DRIVE_FIXED /*|| iRes == DRIVE_REMOTE || iRes == DRIVE_CDROM || iRes == DRIVE_REMOVABLE*/)		
		{
			...
			else
			{
				...
				{
					strDiskPtr[1] = 0;
					int iSize = lpwsprintfA(szBufPtr,",%s盘容量:%I64dGB,剩余容量:%I64dGB",strDiskPtr,liTotalBytes.QuadPart/1000000000,liFreeBytesLeast.QuadPart/1000000000);
					szBufPtr += iSize;
	...
	int len = lpwsprintfA(szBuf,"磁盘总容量:%I64dGB%s",iTotalSize.QuadPart/1000000000,szpartitionspace);
	return TRUE;
}