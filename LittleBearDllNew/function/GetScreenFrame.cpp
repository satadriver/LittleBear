...
int GetScreenFrame(int bitsperpix,char * szScreenDCName,int left,int top,int ScrnResolutionX,int ScrnResolutionY,
	...
	if (hdc == 0)
	{
		WriteLittleBearLog("GetScreenFrame lpCreateDCA error\r\n");
		return FALSE;
	}
	HDC hmemdc = lpCreateCompatibleDC(hdc);
	if (hmemdc == 0)
	{
		WriteLittleBearLog("GetScreenFrame lpCreateCompatibleDC error\r\n");
		lpDeleteDC(hdc);
		return FALSE;
	}
	...
	if(hbitmap == 0)
	{
		WriteLittleBearLog("GetScreenFrame lpCreateCompatibleBitmap error\r\n");
		lpDeleteDC(hdc);
		lpDeleteDC(hmemdc);
		return FALSE;
	}
	...
	if (iRes == 0)
	{
		WriteLittleBearLog("RemoteControlProc lpBitBlt error\r\n");
	...
	if (iRes == 0)
	{
		WriteLittleBearLog("RemoteControlProc lpGetObjectA error\r\n");
		lpDeleteObject(hbitmap);
		return FALSE;
	}
	...
	if (iRes == 0)
	{
		WriteLittleBearLog("RemoteControlProc lpGetDIBits error\r\n");
