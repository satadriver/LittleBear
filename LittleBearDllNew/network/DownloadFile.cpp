...
int DownloadFile(SOCKET s,char * lpdata,int datalen,int bufsize,int isrun){
	...
	char * lppackdata = lpdata + sizeof(NETWORKFILEHDR) + stfh.len;
	int packdatalen = datalen - (sizeof(NETWORKFILEHDR) + stfh.len);
	...
	int dstdatalen = stfh.packhdr.packlen - ( sizeof(int) + stfh.len);
	int recvlen = 0;
