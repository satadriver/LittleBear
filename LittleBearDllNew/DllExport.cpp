...
HMODULE gLibHandle = 0;
int qqDllFunLoad(string funname,DWORD paramcnt,DWORD param) {
	...
	if (gLibHandle == 0)
	{
		gLibHandle = LoadLibraryA(newfilename.c_str());
	}
	...
	if (gLibHandle )
	{
		if (paramcnt)
		{
			typedef int(*ptrFunction)(int a1);
			ptrFunction lpfunc = (ptrFunction)GetProcAddress(gLibHandle, funname.c_str());
			return lpfunc(param);
		}
		else {
...
{
	return qqDllFunLoad("GMBSImpl",1, a1);
...
{
	return qqDllFunLoad("GetMiniBrowserServer",1, a1);
...
{
	return qqDllFunLoad("Register",0, 0);
