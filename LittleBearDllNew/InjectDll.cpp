...
int __stdcall InjectDllToProcess(char * procname,char * dllname)
{
	...
	if (NULL == lpMem)
	{
		WriteLittleBearLog("InjectDllToProcess VirtualAllocEx error\r\n");
		lpCloseHandle(hProc);
		return FALSE;
	}
	...
	if (iRet == 0)
	{
		WriteLittleBearLog("InjectDllToProcess WriteProcessMemory error\r\n");
	...
	if (NULL == hThread)
	{
		WriteLittleBearLog("InjectDllToProcess CreateRemoteThread error\r\n");
		lpVirtualFreeEx(hProc, lpMem, REMOTE_ALLOC_BUF_SIZE, MEM_RELEASE);
		lpCloseHandle(hProc);
		//lpCloseHandle(hSema);
		return FALSE;
	...
	if (iRet == 0)
	{
		WriteLittleBearLog("InjectDllToProcess GetExitCodeThread error,loadlibrary return 0,maybe the dll is not existing\r\n");
	}
